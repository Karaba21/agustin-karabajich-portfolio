...
const technologies = [
  "React 18", "Next.js", "TypeScript", "Tailwind", "Framer Motion",
  "Three.js", "Node.js", "Supabase", "Figma", "GraphQL"
];

export const TechStackCard: React.FC = () => {
  return (
    <div className="h-full bg-white rounded-[2rem] p-8 flex flex-col relative overflow-hidden border border-slate-100 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.02)] hover:shadow-lg transition-shadow duration-500 group min-h-[300px]">
      <div className="flex items-center justify-between mb-8 z-10">
        ...
        <Layers className="text-slate-300 group-hover:text-indigo-500 transition-colors duration-300" size={24} />
      </div>
      ...
      <div className="flex flex-wrap gap-3 z-10 content-start">
        {technologies.map((tech, index) => (
          <span 
            key={`${tech}-${index}`}
            className={`px-4 py-2 rounded-xl text-sm font-bold italic border uppercase tracking-wide transition-colors duration-300 cursor-default ${
               index >= 5 
